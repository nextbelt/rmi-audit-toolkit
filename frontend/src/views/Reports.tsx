...
export const Reports: React.FC = () => {
  ...
  const handleDownload = async (id: number, org: string, site: string) => {
    setError(null);
    setBusyId(id);
    try {
      await generateAndDownloadReport(id, org, site);
    } catch (e: any) {
      setError(
        e?.response?.data?.detail ||
          'Could not generate the report. Make sure the assessment has been scored.',
      );
    } finally {
      setBusyId(null);
    }
    ...
    <div className="page">
      <div className="pg-head">
        <div>
          ...
          <div className="pg-sub">
            <span>Generate and download branded executive PDFs for scored assessments</span>
          </div>
        </div>
      </div>
      ...
      {scored.length === 0 ? (
        ...
        <section className="table-card">
          ...
          {scored.map((a) => (
            <div key={a.id} className="a-row" style={{ gridTemplateColumns: 'minmax(0, 1.6fr) 90px 100px 230px' }}>
              <div>
              ...
              <div className="row" style={{ gap: 8, justifyContent: 'flex-end' }}>
                ...
                <button
                  ...
                  onClick={() => handleDownload(a.id, a.organization_name, a.site_name)}
                >
                  {busyId === a.id ? 'Generating…' : 'Download PDF'}
                </button>
              </div>
            </div>
          ))}
